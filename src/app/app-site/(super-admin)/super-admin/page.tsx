                ...
                supportTickets.map(t => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 overflow-hidden ${!t.logo ? 'bg-gray-700' : 'bg-white'}`}>
                      {t.logo ? <Image src={t.logo} alt="Logo" width={40} height={40} className="w-full h-full object-contain" /> : <UserIcon size={18} className="text-gray-400" />}
                    </div>
                        ...
                        <div className={`flex gap-3 max-w-[80%] ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1 overflow-hidden ${isMe ? "bg-blue-600 text-white" : "bg-gray-700 text-white"}`}>
                            {isMe ? (
                              ...
                              selectedTicket?.logo ? <Image src={selectedTicket.logo} alt="Logo" width={32} height={32} className="w-full h-full object-cover" /> : <UserIcon size={14} />
                            )}
                          </div>
