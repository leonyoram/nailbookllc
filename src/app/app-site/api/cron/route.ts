    ...
    for (const booking of upcomingBookings) {
      try {
        ...
        if (diffHours > 0 && diffHours <= 24) {
          ...
          const template = (tenant.smsTemplates as any)?.reminder || "Reminder: Your appointment for %service_name% at %tenant_name% is on %appointment_start_time%. See you soon!";
    ...
    for (const booking of recentlyCompleted) {
      try {
        ...
        if (diffHours >= 2 && diffHours <= 24) {
          const template = (tenant.smsTemplates as any)?.review || "Thank you for visiting %tenant_name%! We hope you enjoyed your %service_name%. Please leave us a review: " + tenant.googleReviewUrl;
