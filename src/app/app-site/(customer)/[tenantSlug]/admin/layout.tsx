...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    { name: t("admin.dashboard"), href: `/${tenantSlug}/admin`, icon: LayoutDashboard },
    { name: t("admin.calendar"), href: `/${tenantSlug}/admin/calendar`, icon: CalendarDays },
    ...
    { name: t("admin.customers"), href: `/${tenantSlug}/admin/customers`, icon: User },
    { name: t("admin.services"), href: `/${tenantSlug}/admin/services`, icon: Sparkles },
    { name: t("admin.staff"), href: `/${tenantSlug}/admin/staff`, icon: Users, featureId: "staff" },
    ...
    { name: t("admin.reviews"), href: `/${tenantSlug}/admin/reviews`, icon: Star },
    { name: "Support & Feedback", href: `/${tenantSlug}/admin/support`, icon: MessageSquare },
    { name: t("admin.settings"), href: `/${tenantSlug}/admin/settings`, icon: Settings },
  ...
  return (
    ...
    >
      ...
      {/* Sidebar - hidden on mobile when Bottom Nav is used, except when opened as a drawer */}
      <div className={`
        ...
        <div className={`py-5 flex items-center px-6 border-b justify-between ${isDarkMode ? "border-zinc-800" : "border-gray-200"}`}>
          <div className="flex items-center overflow-hidden">
            <div className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-xl mr-3 shrink-0 overflow-hidden border-2 border-primary/20 ${!tenant?.logo ? 'bg-primary text-white' : 'bg-white'}`}>
              {tenant?.logo ? (
                <Image src={tenant.logo} alt="Logo" width={44} height={44} className="w-full h-full object-contain p-0.5" />
              ) : (
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header className={`h-16 border-b flex items-center justify-between px-4 md:px-6 sticky top-0 z-50 transition-colors duration-300 ${isDarkMode ? "bg-black border-zinc-800" : "bg-white border-gray-200"}`}>
          {/* Hide hamburger menu button on mobile since we have bottom nav, except maybe we keep it or replace it */}
          <button 
            onClick={() => setIsMobileMenuOpen(true)}
            className={`p-2 -ml-2 md:hidden rounded-lg transition-colors hidden ${isDarkMode ? "text-zinc-400 hover:bg-zinc-900" : "text-gray-500 hover:bg-gray-100"}`}
          >
            <Menu size={24} />
          </button>
          ...
          <h1 className={`font-semibold md:hidden truncate ${isDarkMode ? "text-white" : "text-gray-800"}`}>Dashboard</h1>
          ...
          <div className="flex items-center gap-2 md:gap-4 ml-auto relative">
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <>
                  <div className="fixed inset-0 z-40 bg-black/20 md:bg-transparent" onClick={() => setShowNotifications(false)}></div>
                  <div className={`fixed inset-x-0 top-16 bottom-0 md:absolute md:top-full md:bottom-auto md:right-0 md:left-auto md:mt-2 w-full md:w-80 md:rounded-xl shadow-2xl border-t md:border overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 flex flex-col ${
                    ...
                    <div className={`p-4 border-b flex justify-between items-center ${isDarkMode ? "bg-zinc-900/50 border-zinc-800" : "bg-gray-50/50 border-gray-50"}`}>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button className="md:hidden text-gray-500" onClick={() => setShowNotifications(false)}><X size={18} /></button>
                      </div>
                    </div>
            ...
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium uppercase shrink-0 overflow-hidden ring-2 ring-primary/20 ${!tenant?.logo ? 'bg-primary text-white' : 'bg-white'}`}>
              {tenant?.logo ? (
                <Image src={tenant.logo} alt="Logo" width={32} height={32} className="w-full h-full object-contain p-0.5" />
              ) : (
        ...
        {/* Page Content */}
        {/* Add padding-bottom on mobile to account for bottom nav (h-16 = 4rem = 64px approx) */}
        <main className="flex-1 p-4 md:p-6 pb-20 md:pb-6 overflow-y-auto">
          {children}
        </main>
      </div>

      <MobileBottomNav isDarkMode={isDarkMode} onMoreClick={() => setIsMobileMenuOpen(true)} />
    </div>
  );
}
